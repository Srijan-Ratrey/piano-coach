...
| 21 | **A step's whole chord is required, and difficulty is how you get around it** | Verification stays all-or-nothing per step: every pitch class must sound. Playability is handled by **thinning the notes** (#per-hand difficulty), not by partially accepting a chord. | Prompted by a report of "I press C5 but it wants C6". It was neither: the piano used the same C4-middle-C convention as the app, and the verifier compares **pitch classes**, so C5 and C6 are literally the same question — an octave-specific refusal is impossible. The real cause was a four-note right-hand step in a downloaded file, which one note can never clear. Partial credit was rejected as the fix: accepting a subset of a chord would mean advancing on notes the player never played, which is the failure #8 exists to prevent and the one the sweep ranks above all others. Instead the app now (a) states its target in plain pitch classes with live per-note indicators, so a stalled step says what it is waiting for, and (b) offers thinning. The underlying lesson: the app was correct and silent, and silence got read as a bug. |
...
| Per-hand difficulty | **Built.** Set before the session; thins the notes shown for that hand. `Melody` = one note per step (highest for the right hand, lowest for the left), `Simplified` = the outer two, `Full` = as written. Its stated purpose was easing a learner in; the sharper one turned out to be **making third-party MIDI playable at all**. The verifier requires every pitch class of a step to sound before advancing, so a downloaded arrangement with five-note right-hand chords is not merely hard — it is unclearable one note at a time, and reads to the player as the app refusing a correct note. Dense chords are also where detection is weakest. See #21. |
